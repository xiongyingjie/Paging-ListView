...
namespace xyj.Lib
{
    public static class ReflectionHelper
    {
        ...
        #region 获取Form表单中提交的键值对
        /// <summary>
        /// 获取Form表单中提交的键值对
        /// </summary>
        /// <param name="FullobjName">对象的类全名</param>
        /// <returns></returns>
        public static string[][] GetFormInfo(string FullobjName)
        {
            //根据名称获取属性
            string[] Properties = GetProperties(FullobjName);
            //存储属性值
            string[] Values = new string[Properties.Length];
            for (int i = 0; i < Properties.Length; i++)
            {
                Values[i] = (HttpContext.Current.Request.Form[Properties[i]] == null) ? "-1" : HttpContext.Current.Request.Form[Properties[i]];
            }
        ...
        /// <param name="nameSpace">对象的命名空间</param>
        /// <returns></returns>
        public static string[][] GetFormInfo(string objName, string nameSpace)
        {
            return GetFormInfo(nameSpace + "." + objName);
        }
        #endregion


        #region 根据键值对转化为的对象  (实例化对象并给对象赋值)
        /// <summary>
        /// 获取表单中提交的对象
        /// </summary>
        /// <param name="FullobjName">对象的类全名</param>
        /// <returns>赋值后的对象</returns>
        public static object GetFormObject(string FullobjName)
        {
            //实例化对象
            object obj = Assembly.GetExecutingAssembly().CreateInstance(FullobjName);
            //获取对象属性集合
            string[] Properties = GetProperties(FullobjName);
            //获取对象值数组
            string[] Values = GetFormInfo(FullobjName)[1];
            //给对象的属性挨个赋值
            for (int i = 0; i < Properties.Length; i++)
            {
                var Property = obj.GetType().GetProperty(Properties[i]);
                //判断属性类型
                if (Property.PropertyType.Name.Contains("Int"))
                {
                    Property.SetValue(obj, int.Parse(Values[i]));
                }
                else if (Property.PropertyType.Name.Contains("Float"))
                {
                    Property.SetValue(obj, float.Parse(Values[i]));
                }
                else
                {
        ...
        /// <param name="nameSpace">对象的命名空间</param>
        /// <returns>赋值后的对象</returns>
        public static object GetFormObject(string objName, string nameSpace)
        {
            return GetFormObject(nameSpace + "." + objName);
        }
        #endregion
